...
import heladeria.Descuento;
import java.sql.*;


public class PedidoComboPersonalizado extends Pedido implements Descuento{
    private int IDProducto;

    public PedidoComboPersonalizado(int IDProducto, int total, int IDPedido, int cantidad) {
        super(total, IDPedido, cantidad);
        this.IDProducto = IDProducto;
    }

    public PedidoComboPersonalizado() {
    ...
    public void pedidoPersonalizado(String combinacion){
                Connection conexion = ConexionDB.conectar(); 
            try {
                String insertQuery = "INSERT INTO productos (nombre, precio, tipo) VALUES (?, 300, 'Combo')";

                PreparedStatement preparedStatement = conexion.prepareStatement(insertQuery, Statement.RETURN_GENERATED_KEYS);
                preparedStatement.setString(1, combinacion);
                ...
                if (Pedido > 0) {
                    ...
                    if (generatedKeys.next()) {
                        ...
                        System.out.println("Prodcuto insertado correctamente. ID del producto: " +IDProducto);
                    }
                } else {
                    System.out.println("No se pudieron insertar los datos.");
                }
     ...
     public void PedidoProducto(int cantidad){
        Connection conexion = ConexionDB.conectar(); 
        try {
    ...
    public double aplicarDescuento(double precioOriginal) {
        if (cantidad > 2) {
            System.out.println("Descuento del 10%. Compra desde 3 helados.");
